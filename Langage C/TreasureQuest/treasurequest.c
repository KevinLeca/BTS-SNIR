        ...
        for (x = 0; x < LARGEUR; x++) {
            ...
            if ((x != posX) || (y != posY)) {
                if (tab[x][y] != DEJAVISITE) {
                    ...
                    printf("*"); //Les case déjà visité deviennent des etoiles
                }
...
void afficherArmeDeuxMains(armesDeuxMains arme) //Affiche l'arme et les dégats selon l'arme en mains
{
    switch(arme){
        case BATARDE:
...
int calculDegats(typePersonnage *attaquant, typePersonnage *defenseur) //calcul et retourne les degats
{
    int atck, def, degats;
    atck = calculAttaque(attaquant);    //calcul de l'attaque
    def = calculDefense(defenseur);     //calcul de la defense
    degats = atck - def;                //calcul des degats

    if(degats<=0){          //Si les degats sont <= 0
        degats = 1;
    }

    return degats;          //On retourne les degats
}
...
void faireUnDuel(typePersonnage *j1, typePersonnage *j2) {
    ...
    do {
     ...
     if(j1->pv > 0 && j2->pv <= 0){   //Si le joueur 1 l'emporte
        if(j1->pv + 100 > j1->pvMax){ //Remet ses pv à pvMax si pv+100 dépasse pvMax
            j1->pv = j1->pvMax;
        }
        else{                         //Ajoute 100 aux pv sinon
            j1->pv = j1->pv + 100;
        }
        j1gagne = TRUE;
     }

     else{                            //Sinon c'est que le joueur est à 0pv
        j1->pv = 0;
        j1gagne = FALSE;
     }
    ...
    mettreAJourNiveau(j1, exp); // Met a jour le niveau du joueur
...
void afficherPersonnage(typePersonnage *personnage){
    printf("Classe : ");
    afficherClasse(personnage->competence); //Affiche la classe
    printf("Nom : %s \n", personnage->nom);
    printf("Prenom : %s \n", personnage->prenom);
    printf("Age : %i \n", personnage->age);
    printf("Taille : %.2f \n", personnage->taille);
    printf("Poids : %.2f \n", personnage->poids);
    printf("Resistance : %i \n", personnage->resistancePhysique);
    printf("Points de vie : [%i/%i] \n", personnage->pv, personnage->pvMax);
    printf("Niveau : %i \n", personnage->niveau);
    printf("Experience dans le niveau : %i/200\n", personnage->experience);
    afficherArmement(personnage->armes); //Affiche l'equipement
}
